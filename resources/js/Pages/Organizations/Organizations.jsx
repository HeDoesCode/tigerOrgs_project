...
function Organizations() {
    ...
    function ControlKeywords({ keywordList }) {
                ...
                <Dialog>
                    ...
                    <DialogContent className="sm:max-w-[425px]">
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                Filter organizations by adding or removing
                                keywords. Save changes when you're done.
                            </DialogDescription>
                        </DialogHeader>
        ...
        function KeywordTile({ name }) {
            return (
                <div className="w-min px-2 py-1 bg-gray-200 border border-gray-300 h-fit rounded-md cursor-pointer">
                    {name}
                </div>
            );
        }
    ...
    function OrgJoined({ icon, title, isAdmin, isSuperAdmin, link }) {
        return (
            <Link href={link}>
        ...
        return (
            <div className={`flex flex-col space-y-3 w-full ${className}`}>
                <div className="flex flex-row justify-between -mb-3">
                    <div className="questrial font-bold tracking-wider">
                        {title}
                    </div>
                    <div className="w-20 relative overflow-visible">
                        <button className="absolute right-0 min-w-max underline text-sm py-1 px-2 text-gray-500 hover:bg-gray-800 hover:text-white rounded-lg">
                            show all
                        </button>
                    </div>
                </div>
    ...
    function OrganizationTile({
        ...
        return (
            <Link
                href={href}
                className={`w-min space-y-2 hover:cursor-pointer group p-3 hover:scale-[1.06] flex flex-col justify-center items-center transition-all duration-300 ease-in-out transOptimize h-max select-none ${className}`}
            >
                <div className="h-52 md:h-72 aspect-[5/8] relative">
                    <img
                        src={orgBg}
                        className="object-cover h-full rounded-lg shadow-sm"
                        alt={orgBg}
                        onError={hideImage}
                    />
                    {isRecruiting && (
                    ...
                    <img
                        src={orgIcon}
                        className="absolute right-2 bottom-2 bg-[#EEEEEE] size-14 rounded-full object-cover"
                    />
                    <div className="transition-all hidden md:flex flex-col justify-center duration-200 ease-in-out absolute inset-0 bg-black/0 overflow-y-auto group-hover:bg-black/70 invisible group-hover:visible group-hover:delay-100 rounded-lg text-white/0 group-hover:text-white/100 p-3 tracking-wide transOptimize">
                        <p className="poppins tracking-normal md:tracking-wide text-sm md:text-lg">
                            {title}
                        </p>
                        <p className="quicksand font-extralight text-sm overflow-hidden overflow-ellipsis">
                            {desc}
                        </p>
                    </div>
                </div>
                ...
                <div className="space-y-1">
                    <div className="w-full line-clamp-2 poppins text-sm font-bold tracking-wide">
                        {title}
                    </div>
                    <div className="w-full text-xs line-clamp-2">{desc}</div>
                    <div className="w-full text-xs line-clamp-1 text-gray-600 truncate">
                        #### Followers
                    </div>
                </div>
            </Link>

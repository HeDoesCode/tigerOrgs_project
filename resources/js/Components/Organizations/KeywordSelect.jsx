...
function KeywordSelect({ keywords, className, activeUserKeywords }) {
    ...
    const resetKeywords = () => {
        ...
        const filteredKeywords = keywords.filter(
            (keywordObj) =>
                !activeUserKeywords.some(
                    (activeKeywordObj) =>
                        activeKeywordObj.keyword === keywordObj.keyword
                )
        );
    ...
    const handleSearchChange = (e) => {
        ...
        const filteredKeywords = keywordBank
            ...
            .sort((a, b) => {
                ...
                const aStartsWith = aKeyword.startsWith(keywordFilterLower)
                    ? 0
                    : 1;
                const bStartsWith = bKeyword.startsWith(keywordFilterLower)
                    ? 0
                    : 1;
                ...
                return (
                    aKeyword.indexOf(keywordFilterLower) -
                        bKeyword.indexOf(keywordFilterLower) ||
                    aKeyword.localeCompare(bKeyword)
                );
            });

        const nonMatchingKeywords = keywordBank
            .filter(
                (item) =>
                    !item.keyword
                        .toLowerCase()
    ...
    const enableKeyword = (item) => {
        ...
        setKeywordBank((prevKeywordBank) => {
            return prevKeywordBank.filter(
                (prevItem) => prevItem.keyID !== item.keyID
            );
        });
    };
    ...
    return (
        <div
            className={`w-full flex flex-wrap gap-2 min-h-10 border-[1px] rounded-md p-2 relative text-xs overflow-clip group ${className}`}
        >
            <Dialog>
                <DialogTrigger className="absolute size-full inset-0">
                    <div className="size-full flex items-center justify-center invisible group-hover:visible group-hover:!bg-gray-800/70 text-black/0 group-hover:!text-white transition-all duration-200 ease-in-out">
                        {(activeKeywords && "Edit") || "Add"}&nbsp;Keyword
                        Filters
                    </div>
                </DialogTrigger>
                <DialogContent className="sm:max-w-[32rem] !h-fit max-h-[90%] w-[90%] sm:h-auto overflow-y-auto select-none">
                    <DialogHeader>
                        <DialogTitle>Add/Remove Keyword Filters</DialogTitle>
                        <DialogDescription className="!-mb-3">
                            Filter organizations by adding or removing keywords.
                    ...
                    <div className="flex gap-x-3">
                        ...
                        <button
                            ...
                            className="h-full px-3 border border-black rounded-md hover:bg-gray-800 hover:text-white transition-all"
                        >
                            Reset
                        </button>
                    </div>
            ...
            {activeUserKeywords.length !== 0 ? (
                ...
                <div className="text-gray-500 my-auto flex items-center">
                    (None Selected. We'll use this to recommend organizations to
                    you.)
                </div>
            )}
        </div>
    ...
    function KeywordTile({ name }) {
        return (
            <div className="w-min px-2 py-1 bg-[#ffb700] border border-gray-300 h-fit rounded-md cursor-pointer whitespace-nowrap">
                {name}
            </div>
        );
    }
    ...
    function EditableKeywordTile({ name, remove, add, ...props }) {
        return (
            <button
                {...props}
                className={`select-none w-min px-2 py-1 ${
                    add && "bg-gray-200"
                } ${
                    remove && "bg-[#ffb700]"
                } border border-gray-300 h-8 rounded-md space-x-2 flex items-center ${
                    add && "hover:bg-gray-300"
                } ${remove && "hover:bg-[#e6a70b]"} group`}
            >
                ...
                <div className="h-5 aspect-square text-gray-400">
                    {remove && (
                        <svg
                            ...
                            viewBox="0 0 24 24"
                            fill="currentColor"
                            className="icon icon-tabler icons-tabler-filled icon-tabler-circle-x text-black/70"
                        >
                            <path stroke="none" d="M0 0h24v24H0z" fill="none" />

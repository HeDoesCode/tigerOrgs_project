...
function OrganizationContainerRow({
    children,
    title,
    className,
    index,
    ...
    return (
        <div
            className={`flex flex-col gap-y-3 w-full relative ${className} ${
                expand && expandSpacing(index)
            }`}
        >
            <div className="flex flex-row justify-between -mb-3">
                ...
                <div className="w-20 min-w-20 relative overflow-visible">
                    {(hasHorizontalOverflow || expand) && (
                        <button
                            onClick={expandRow}
                            className={`absolute right-0 min-w-max underline text-sm py-1 px-2 hover:bg-gray-800 hover:text-white rounded-lg ${
                                expand
            ...
            {!expand && hasHorizontalOverflow && (
                <div
                    className={`absolute right-0 top-6 bottom-0 w-20 bg-gradient-to-r from-transparent to-[#EEEEEE]/70 pointer-events-none transition-all ease-in ${
                        isScrolledToEnd && "!w-0"
                    }`}
                ></div>
            )}
            {!expand && hasHorizontalOverflow && (
                <div
                    className={`absolute left-0 top-6 bottom-0 w-20 bg-gradient-to-l from-transparent to-[#EEEEEE]/70 pointer-events-none transition-all ease-in ${
                        isScrolledOffStart && "!w-0"
                    }`}
                ></div>
            )}
            {hasHorizontalOverflow && expand && (
                <div className="w-full flex justify-end">
                    <button
                        onClick={expandRow}
                        className={`underline text-sm py-1 px-2 hover:bg-gray-800 hover:text-white rounded-lg text-blue-500 font-bold`}
                    >
                        hide all
                    </button>
                </div>

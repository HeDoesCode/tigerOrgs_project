...
function OrganizationJoined({ icon, title, isAdmin, isSuperAdmin, link, visibility }) {
    return (
    ...
    function Label({ children, className }) {
        return (
            <label className={`cursor-pointer text-white text-[0.6rem] w-fit px-[0.35rem] rounded-full ${className}`}>
                {children}
            </label>
        )
    }

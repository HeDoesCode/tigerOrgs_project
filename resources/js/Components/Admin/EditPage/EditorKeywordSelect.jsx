...
function EditorKeywordSelect({ keywords, className, activeOrgKeywords, updateOrgKeywords }) {
    ...
    useEffect(() => {
        setUpdateButtonVisible(activeKeywords !== activeOrgKeywords);
    }, [activeKeywords])

    const resetKeywords = () => {
        setActiveKeywords(activeOrgKeywords);

        const filteredKeywords = keywords.filter(
            keywordObj => !activeOrgKeywords.some(
    ...
    return (
        <div className={`w-full flex flex-wrap gap-2 min-h-10 border-[1px] rounded-md p-2 relative text-xs overflow-clip group ${className}`}>
            <Dialog>
                <DialogTrigger className="absolute size-full inset-0">
                    <div className="size-full flex items-center justify-center invisible group-hover:visible group-hover:!bg-gray-800/70 text-black/0 group-hover:!text-white transition-all duration-200 ease-in-out">
                        {activeKeywords?.length !== 0 && 'Edit' || 'Add'}&nbsp;Keywords
                    </div>
                </DialogTrigger>
                        ...
                        <DialogClose asChild>
                            <div className="w-full flex justify-end space-x-3">
                                ...
                                <Button
                                    ...
                                    onClick={() => updateOrgKeywords(activeKeywords)}
                                >
                                    Update
                                </Button>
                            </div>
            ...
            {activeOrgKeywords.length !== 0
                ? activeOrgKeywords.map((item, index) => (
...
export default EditorKeywordSelect
